...
void USceneCapturer::InitCaptureComponent( USceneCaptureComponent2D* CaptureComponent, float HFov, float VFov, EStereoscopicPass InStereoPass )
{
    ...
    FString tname = FString::Printf(TEXT("TextTarg%f_%f"), rnd, rnd2);
    CaptureComponent->TextureTarget = CreateDefaultSubobject<UTextureRenderTarget2D>(FName(*tname));
  ...
  //CaptureComponent->TextureTarget->InitAutoFormat(CaptureWidth, CaptureHeight);
  //CaptureComponent->TextureTarget->bForceLinearGamma = false;
	//CaptureComponent->TextureTarget->bHDR = false;

	FPostProcessSettings pps;
	pps.VignetteIntensity = 0;
	pps.bOverride_VignetteIntensity = true;
	pps.LensFlareIntensity = 0;
	pps.bOverride_LensFlareIntensity = true;
	pps.SceneFringeIntensity = 0;
	pps.bOverride_SceneFringeIntensity = true;
	pps.BloomIntensity = 0;
	pps.bOverride_BloomIntensity = true;
	//pps.AutoExposureBias = 1.0; //fix brightness
...
USceneCapturer::USceneCapturer(FVTableHelper& Helper)
    ...
    , hAngIncrement(FStereoPanoManager::HorizontalAngularIncrement->GetFloat())
    , vAngIncrement(FStereoPanoManager::VerticalAngularIncrement->GetFloat())
    , eyeSeparation(FStereoPanoManager::EyeSeparation->GetFloat())
    ...
    , SphericalAtlasWidth(FStereoPanoManager::StepCaptureWidth->GetInt())
    , SphericalAtlasHeight(SphericalAtlasWidth / 2)
    , bForceAlpha(FStereoPanoManager::ForceAlpha->GetInt() != 0)
    , bEnableBilerp(FStereoPanoManager::EnableBilerp->GetInt() != 0)
    , SSMethod(FMath::Clamp<int32>(FStereoPanoManager::SuperSamplingMethod->GetInt(), 0, ARRAY_COUNT(g_ssPatterns)))
    , bOverrideInitialYaw(FStereoPanoManager::ShouldOverrideInitialYaw->GetInt() != 0)
    , ForcedInitialYaw(FRotator::ClampAxis(FStereoPanoManager::ForcedInitialYaw->GetFloat()))
    , OutputDir(FStereoPanoManager::OutputDir->GetString().IsEmpty() ? FPaths::GameSavedDir() / TEXT("StereoPano") : FStereoPanoManager::OutputDir->GetString())
    //, dbgDisableOffsetRotation(FStereoPanoManager::FadeStereoToZeroAtSides->GetInt() != 0)
    , fadeStereoToZeroByVertical( FStereoPanoManager::FadeStereoToZeroByVertical->GetInt() != 0)
{}
    ...
    , fadeStereoToZeroByVertical( FStereoPanoManager::FadeStereoToZeroByVertical->GetInt() != 0)
{
    ...
    if (dbgMatchCaptureSliceFovToAtlasSliceFov)
    {
        ...
        captureHFov = FStereoPanoManager::CaptureHorizontalFOV->GetFloat();
    ...
    else
    {
        ...
        captureVFov = captureHFov = FStereoPanoManager::CaptureHorizontalFOV->GetFloat();
        ...
        CaptureHeight = CaptureWidth = FStereoPanoManager::CaptureSlicePixelWidth->GetInt();
	...
	for( int CaptureIndex = 0; CaptureIndex < FStereoPanoManager::ConcurrentCaptures->GetInt(); CaptureIndex++ )
	{
...
void USceneCapturer::SetPositionAndRotation( int32 CurrentHorizontalStep, int32 CurrentVerticalStep, int32 CaptureIndex )
{
    ...
    */
    ...
    //decrease offset for top and bottom
    if (fadeStereoToZeroByVertical)
    {
        float eyeSeparationDampeningFactor = 1.0f;
        if (Rotation.Pitch <= 180.0f)
        {
            eyeSeparationDampeningFactor = FMath::Lerp(0.0f, 1.0f, fabs(Rotation.Pitch - 90.0f) / 90.0f);
        }
        else
        {
            eyeSeparationDampeningFactor = FMath::Lerp(0.0f, 1.0f, fabs(Rotation.Pitch - 270.0f) / 90.0f);
        }

        Offset = Rotation.RotateVector(Offset * eyeSeparationDampeningFactor);
    }
    else
    {
      Offset = Rotation.RotateVector(Offset);
...
void USceneCapturer::ValidateParameters()
{
	...
	{
		UE_LOG( LogStereoPano, Warning, TEXT( "Horizontal angular step (%g) is not a factor of 360! This will lead to a seam between the start and end points" ), hAngIncrement );
	}

	if( ( int32 )( (NumberOfVerticalSteps - 1) * vAngIncrement ) != 180 )
	{
		UE_LOG( LogStereoPano, Warning, TEXT( "Vertical angular step (%g) is not a factor of 180! This will lead to a seam between the start and end points" ), vAngIncrement );
	}

	TotalSteps = NumberOfHorizontalSteps * NumberOfVerticalSteps;
    if( ( SphericalAtlasWidth & 1 ) != 0)
    {
        UE_LOG(LogStereoPano, Warning, TEXT("The Atlas Width (%d) must be even! Otherwise the Atlas height will not divide evenly."), SphericalAtlasWidth);
    }
	...
	if( ( StripWidth & 1 ) != 0 )
	{
		UE_LOG( LogStereoPano, Warning, TEXT( "Strip width (%d) needs to be even to avoid bad offsets" ), StripWidth );
	}

	if( StripWidth * NumberOfHorizontalSteps != SphericalAtlasWidth )
	{
		UE_LOG( LogStereoPano, Warning, TEXT( "The number of horizontal steps (%d) needs to be a factor of the atlas width (%d)" ), NumberOfHorizontalSteps, SphericalAtlasWidth );
	}

    if ((StripHeight & 1) != 0)
    {
        UE_LOG(LogStereoPano, Warning, TEXT("Strip height (%d) needs to be even to avoid bad offsets"), StripHeight);
    }

    if (StripHeight * (NumberOfVerticalSteps - 1) != SphericalAtlasHeight)
	{
        UE_LOG(LogStereoPano, Warning, TEXT("The number of vertical steps (%d) needs to be a factor of the atlas height (%d)"), NumberOfVerticalSteps, SphericalAtlasHeight);
	}

    //TODO: ikrimae: Validate capturewidth & captureheight. Need to be even

	UE_LOG( LogStereoPano, Display, TEXT( "Stereo panoramic screenshot parameters" ) );
	UE_LOG( LogStereoPano, Display, TEXT( " ... capture size: %d x %d" ), CaptureWidth, CaptureHeight );
	UE_LOG( LogStereoPano, Display, TEXT( " ... spherical atlas size: %d x %d" ), SphericalAtlasWidth, SphericalAtlasHeight );
    UE_LOG( LogStereoPano, Display, TEXT( " ... intermediate atlas size: %d x %d" ), UnprojectedAtlasWidth, UnprojectedAtlasHeight );
	UE_LOG( LogStereoPano, Display, TEXT( " ... strip size: %d x %d" ), StripWidth, StripHeight );
	UE_LOG( LogStereoPano, Display, TEXT( " ... horizontal steps: %d at %g degrees" ), NumberOfHorizontalSteps, hAngIncrement );
	UE_LOG( LogStereoPano, Display, TEXT( " ... vertical steps: %d at %g degrees" ), NumberOfVerticalSteps, vAngIncrement );
}
...
void USceneCapturer::SetInitialState( int32 InStartFrame, int32 InEndFrame, FStereoCaptureDoneDelegate& InStereoCaptureDoneDelegate )
{
	if( bIsTicking )
	{
		UE_LOG( LogStereoPano, Warning, TEXT( "Already capturing a scene; concurrent captures are not allowed" ) );
		return;
	}
...
TArray<FColor> USceneCapturer::SaveAtlas(FString Folder, const TArray<FColor>& SurfaceData)
{
    ...
    //    Supersample that UV coordinate from the unprojected atlas
    {
        ...
        UE_LOG(LogStereoPano, Log, TEXT("Sampling atlas..."));
        ...
        UE_LOG(LogStereoPano, Log, TEXT("...done! Duration: %g seconds"), SamplingDuration.GetTotalSeconds());
    }
    ...
    if (FStereoPanoManager::GenerateDebugImages->GetInt() != 0)
    {
	...
	UE_LOG( LogStereoPano, Log, TEXT( " ... done!" ), *AtlasName );
...
void USceneCapturer::CaptureComponent( int32 CurrentHorizontalStep, int32 CurrentVerticalStep, FString Folder, USceneCaptureComponent2D* CaptureComponent, TArray<FColor>& Atlas )
{
	...
	if( FStereoPanoManager::GenerateDebugImages->GetInt() != 0 )
	{
		...
		UE_LOG( LogStereoPano, Log, TEXT( "Writing snapshot: %s" ), *CaptureName );
...
void USceneCapturer::Tick( float DeltaTime )
{
	...
	else if( CurrentStep < TotalSteps )
	{
        ...
        {
            FlushRenderingCommands();
            for (int32 CaptureIndex = 0; CaptureIndex < FStereoPanoManager::ConcurrentCaptures->GetInt(); CaptureIndex++)
            {
        ...
        int32 currentPercent = int32(CurrentStep * 1000 / TotalSteps);
        if (currentPercent > LastPercent) {
            UE_LOG( LogStereoPano, Display, TEXT( "Progress: %.1f %%" ), currentPercent / 10.0f );
	...
	else
	{
		...
		UE_LOG( LogStereoPano, Log, TEXT( "Duration: %g seconds for frame %d" ), Duration.GetTotalSeconds(), CurrentFrameCount );
		StartTime = EndTime;
		...
		else
		{
			...
			UE_LOG( LogStereoPano, Log, TEXT("Duration: %g minutes for frame range [%d,%d] "), OverallDuration.GetTotalMinutes(), StartFrame, EndFrame );
